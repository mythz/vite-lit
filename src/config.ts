...
import { JsonServiceClient, createBus, toKebabCase } from "@servicestack/client"
...
export class Sole {
    ...
    static client = signal(new JsonServiceClient())
    static events = createBus()
    static user = signal<AuthenticateResponse|null>(null)
    static metadata = signal<AppMetadata|null>(null)
    static components:{[k:string]:Constructor} = {
        RouterLink,
    }
